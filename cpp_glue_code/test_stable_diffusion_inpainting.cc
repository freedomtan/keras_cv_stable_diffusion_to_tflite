...
vector<float> run_image_encoder(vector<float> latent) {
  vector<float> empty;
  auto model = tflite::FlatBufferModel::BuildFromFile(
...
int main(int argc, char *argv[]) {
  ...
  std::string f = "man-on-skateboard-cropped.rgb";

  auto image = read_raw_image(f);
  cout << "image.size(): " << image.size() << "\n";
  auto encoded_image = run_image_encoder(image);
  cout << "encoded_image.size(): " << encoded_image.size() << "\n";
